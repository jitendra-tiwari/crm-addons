...
function getValidUser()
{
    ...
    try
    {
        ...
        var oDataUri = serverUrl + "/XRMServices/2011/OrganizationData.svc/dots_webformconfigurationSet?$filter=dots_orguniquename eq '" + orgUniqueName + "' and dots_serverurl eq '" + serverUrl + "' ";
...
function SaveConfigurationDetails(RegisterId,serverUrl, orgUniqueName, username, password) {
    ...
    if (serverUrl != "") {
        ...
        var ODATA_EntityCollection = "/dots_webformconfigurationSet";
        ///////////////////////////////////////////////////////////// 
        // Define attribute values for the CRM object you want created 

       
        CRMObject.dots_serverurl = serverUrl;
        CRMObject.dots_registerid = RegisterId;
        CRMObject.dots_orguniquename = orgUniqueName;
        CRMObject.dots_username = username;
        CRMObject.dots_password = password;
...
function UpdateConfigurationDetails(RegisterId, serverUrl, orgUniqueName, username, password, tblconfigId) {
    ...
    var crmUrl = parent.Xrm.Page.context.getClientUrl() + "/XRMServices/2011/OrganizationData.svc/dots_webformconfigurationSet(guid'" + tblconfigId + "')";
    var jsonEntity = window.JSON.stringify(CRMObject);
    //The OData end-point
    var ODATA_ENDPOINT = "/XRMServices/2011/OrganizationData.svc";
...
function CheckConfigurationDetails() {
    ...
    $.ajax({
        ...
        success: function (data, textStatus, XmlHttpRequest) {
            ...
            if (output.length > 0) {
                console.log(output[0].dots_registerid);
                LoadConfigurationService(output[0].dots_registerid);
            }
            else
            {
                $(".loading").hide();
